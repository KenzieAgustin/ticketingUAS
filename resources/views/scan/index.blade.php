...
<body>
    ...
    <div class="tab-panel" id="panel-manual">
        ...
        <input type="text" id="booking-input" placeholder="PRJ2026-XXXXXX" maxlength="13"
    ...
    <div class="tab-panel" id="panel-qr" style="display:none;">
        ...
        <input type="text" id="qr-fallback" placeholder="PRJ2026-XXXXXX" maxlength="13"
               oninput="this.value = this.value.toUpperCase()"
    ...
    <script>
        ...
        function validate(code) {
            if (!code || code.length < 13) {
                showResult('invalid', 'Format booking code tidak valid.');
                return;
            }
            ...
            .then(data => {
                ...
                showResult(status, data.message, data.data);
                addHistory(code, status);
            })
            .catch(() => {
                document.getElementById('loading').style.display = 'none';
                showResult('invalid', 'Gagal terhubung ke server.');
            });
        }
        ...
        function showResult(status, message, data) {
            ...
            document.getElementById('result-area').innerHTML = `
                <div class="result ${status}">
                    ...
                    <span>${message || ''}</span>
                    ${detail}
                    ${status === 'valid' ? '<br><a href="#" onclick="resetScan();return false;" style="font-size:13px;">Scan tiket berikutnya</a>' : ''}
                </div>`;
        }
        ...
        function startCamera() {
            ...
            if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
                navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } })
                    ...
                    .catch(() => showResult('invalid', 'Tidak bisa akses kamera. Gunakan input manual.'));
            } else {
                showResult('invalid', 'Browser tidak mendukung kamera.');
            }
        }
    </script>
